...
public class DotTreeGUI extends DrawingGUI {
	...
	public void handleMousePress(int x, int y) {
		//mode to add new points to quadtree
		if (mode == 'a') {
			//if the tree is empty, create a new tree
			if (tree==null)
			{Dot dot= new Dot(x,y);
				tree= new PointQuadtree<Dot>(dot, 0, 0, width, height);}

			//else, insert the point pressed into the existing tree
			else{tree.insert(new Dot(x,y));}

		}
		//query mode: search for points in a circle near mouse
		else if (mode == 'q') {
			// Set "found" to what tree says is near the mouse press
			//searches in a circle of given radius mouseRadius, centered at mouse press location
	...
	public void drawTree(Graphics g, PointQuadtree<Dot> tree, int level) {
		...
		//create a blob at tree node and draw it
		Blob blob= new Blob(tree.getPoint().getX(), tree.getPoint().getY());
		blob.draw(g);
		//draw vertical and horizontal lines through the blob center, beginning and ending at quadrant boundaries
		g.drawLine((int) tree.getPoint().getX(), tree.getY1(), (int) tree.getPoint().getX(), tree.getY2());
		g.drawLine(tree.getX1(), (int) tree.getPoint().getY(), (int) tree.getX2(), (int) tree.getPoint().getY());

		// Recurse with children
		//check for children in each quadrant
		//if there is a child, draw the child
