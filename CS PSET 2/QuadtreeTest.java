...
public class QuadtreeTest {
	...
	private static void test1() {
 	...
 	//our test method, which checks each method in pointquadtree
	private static void test2() {
		//create the quadtree: tests insert method
		PointQuadtree<Dot> tree = new PointQuadtree<Dot>(new Dot(600,200, "A"), 0,0,800,600); // start with A
		tree.insert(new Dot(410,290, "B"));
		tree.insert(new Dot(350,550, "C"));
		tree.insert(new Dot(650,200, "D"));
		tree.insert(new Dot(700,250, "E"));
		tree.insert(new Dot(450,175, "F"));
		tree.insert(new Dot(600,125, "G"));
		tree.insert(new Dot(775,250, "H"));
		tree.insert(new Dot(325,225, "I"));
		tree.insert(new Dot(390,315, "J"));
		tree.insert(new Dot(100,550, "K"));
		tree.insert(new Dot(410,410, "L"));

		//prints out quadtree. tests toString method and helper method
		System.out.println(tree.toString());

		//prints out all points in tree. tests allPoints method and helper
		System.out.println("Points in tree: " + tree.allPoints());

		//prints out points withing set circle in tree. tests findInCircle and helper method
		System.out.println("Points in center of tree: " + tree.findInCircle(400,300,20));

		//tests whether size and number of points in circle are accurate for the user
		//returns a pass if they are correct
		if (tree.size()==12 && tree.findInCircle(400,300,20).size()==2)
